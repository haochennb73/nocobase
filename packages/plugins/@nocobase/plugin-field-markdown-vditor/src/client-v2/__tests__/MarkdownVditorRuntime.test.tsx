...
describe('MarkdownVditorRuntime', () => {
  ...
  it('uses the bundled vditor assets instead of the public CDN in development', () => {
    vi.stubEnv('NODE_ENV', 'test');
    const getCdnUrl = vi.fn(() => '/admin/');
    const runtime = new MarkdownVditorRuntime({ getCdnUrl } as never, () => '/v2/admin/');

    const expected = '/admin/static/plugins/@nocobase/plugin-field-markdown-vditor/dist/client/vditor';
    expect(runtime.getCDN()).toBe(expected);
    expect(runtime.dependencies).toEqual({
      cdn: expected,
    });
  });

  it('builds the bundled vditor asset URL from the app CDN url in production', () => {
    vi.stubEnv('NODE_ENV', 'production');
    const getCdnUrl = vi.fn(() => '/admin/');
    const runtime = new MarkdownVditorRuntime({ getCdnUrl } as never, () => '/v2/admin/');

    expect(runtime.getCDN()).toBe('/admin/static/plugins/@nocobase/plugin-field-markdown-vditor/dist/client/vditor');
    ...
    expect(requireFn.config).toHaveBeenCalledWith({
      ...
      paths: expect.objectContaining({
        'plugin-field-markdown-vditor-dep.katex':
          '/admin/static/plugins/@nocobase/plugin-field-markdown-vditor/dist/client/vditor/dist/js/katex/katex.min.js?v=0.16.9',
        'plugin-field-markdown-vditor-dep.mermaid':
          '/admin/static/plugins/@nocobase/plugin-field-markdown-vditor/dist/client/vditor/dist/js/mermaid/mermaid.min',
      }),
    });
